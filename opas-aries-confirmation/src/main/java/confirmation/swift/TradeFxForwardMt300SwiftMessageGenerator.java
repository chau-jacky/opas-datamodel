...
public class TradeFxForwardMt300SwiftMessageGenerator extends AbstractTradeMt300SwiftMessageGenerator {

	public TradeFxForwardMt300SwiftMessageGenerator() {
		super();
	}
	
	/**
	 * Format and return Sequence A Field 22C based on the given Forward trade.
	 *
	 * @param trade FX Forward trade that requires generation of MT300
	...
	public Field22C getSequenceAField22C(Trade trade) {
		...
		BigDecimal contractRate = ((TradeFxForward) trade).getContractRate();
		...
		for (ListIterator<String> itr = commonReference.listIterator(); itr.hasNext();) {
			field22C.setComponent(itr.nextIndex() + 1, itr.next());
		}
	...
	/**
	 * Format and return Sequence A Field 17I based on the given Forward trade.
	 *
	 * @param trade FX Forward trade that requires generation of MT300
	...
	public Field17I getSequenceAField17I(Trade trade) {
		...
		boolean pvpIndicator = ((TradeFxForward) trade).isPvpIndicator();
	...
	/**
	 * Format and return Sequence A Field 77D based on the given Forward trade.
	 *
	 * @param trade FX Forward trade that requires generation of MT300
	...
	public Field77D getSequenceAField77D(Trade trade) {
		...
		LegalAgreement legalAgreement = ((TradeFxForward) trade).getLegalAgreement();
	...
	/**
	 * Format and return Sequence A Field 14C based on the given Forward trade.
	 *
	 * @param trade FX Forward trade that requires generation of MT300
	...
	/**
	 * Format and return Sequence A Field 17F based on the given Forward trade.
	 *
	 * @param trade FX Forward trade that requires generation of MT300
	...
	public Field17F getSequenceAField17F(Trade trade) {
		...
		boolean nonDeliverableIndicator = ((TradeFxForward) trade).isNonDeliverableIndicator();
	...
	/**
	 * Format and return Sequence A Field 17O based on the given Forward trade.
	 *
	 * @param trade FX Forward trade that requires generation of MT300
	...
	/**
	 * Format and return Sequence A Field 32E based on the given Forward trade.
	 *
	 * @param trade FX Forward trade that requires generation of MT300
	...
	public Field32E getSequenceAField32E(Trade trade) {
		...
		boolean nonDeliverableIndicator = ((TradeFxForward) trade).isNonDeliverableIndicator();
		Currency boughtCurrency = ((TradeFxForward) trade).getBoughtCurrency();
		Currency soldCurrency = ((TradeFxForward) trade).getSoldCurrency();
	...
	/**
	 * Format and return Sequence A Field 30U based on the given Forward trade.
	 *
	 * @param trade FX Forward trade that requires generation of MT300
	...
	public Field30U getSequenceAField30U(Trade trade) {
		...
		boolean nonDeliverableIndicator = ((TradeFxForward) trade).isNonDeliverableIndicator();
		Date fixingDate = ((TradeFxForward) trade).getFixingDate();
		
		if (nonDeliverableIndicator) {
			
			field30U.setDate(dateFormatter.format(fixingDate));
	...
	/**
	 * Format and return Sequence A Field 14S based on the given Forward trade.
	 *
	 * @param trade FX Forward trade that requires generation of MT300
	...
	public Field14S getSequenceAField14S(Trade trade) {
		...
		String ndfSettlementRateSource = ((TradeFxForward) trade).getNdfSettlementRateSource();
		String ndfSettlementTime = ((TradeFxForward) trade).getNdfSettlementTime();
		String ndfSettlementLocation = ((TradeFxForward) trade).getNdfSettlementLocation();
		
		if (nonDeliverableIndicator) {
		
			field14S.setSource(ndfSettlementRateSource.substring(0, 3));
			field14S.setNumber(ndfSettlementRateSource.substring(3, 5));
			field14S.setTime(ndfSettlementTime);
			field14S.setLocation(ndfSettlementLocation);
	...
	/**
	 * Format and return Sequence A Field 21A based on the given Forward trade.
	 *
	 * @param trade FX Forward trade that requires generation of MT300
	...
	public Field21A getSequenceAField21A(Trade trade) {
		...
		/* This tag would not be required for FX Forward */
	...
	public Field30V getSequenceBField30V(Trade trade) {
		...
		field30V.setDate(dateFormatter.format(((TradeFxForward)trade).getValueDate()));
		
		return field30V;
	}
	
	/**
	 * Format and return Sequence B Field 36 based on the given Forward trade.
	 *
	 * @param trade FX trade that requires generation of MT300
	...
	/**
	 * Format and return Sequence B1 Field 32B based on the given Forward trade.
	 *
	 * @param trade FX trade that requires generation of MT300
	...
	public Field32B getSequenceB1Field32B(Trade trade) {
		...
		TradeFxForward TradeFxForward = (TradeFxForward) trade;

		field32B.setCurrency(TradeFxForward.getBoughtCurrency().getCurrencyCode());
		field32B.setAmount(TradeFxForward.getBoughtCurrencyAmount().abs());
	...
	/**
	 * Format and return Sequence B2 Field 33B based on the given Forward trade.
	 *
	 * @param trade FX trade that requires generation of MT300
	...
	public Field33B getSequenceB2Field33B(Trade trade) {
		...
		field33B.setCurrency(TradeFxForward.getSoldCurrency().getCurrencyCode());
		field33B.setAmount(TradeFxForward.getSoldCurrencyAmount().abs());
	...
	public Field getSequenceB1Field53a(Trade trade) {
		...
		CashFlowSi receiveCashFlowSi = ((TradeFxForward)trade).getCashFlow(PaymentIndicator.RECEIVE).getCashFlowSi();
	...
	public Field getSequenceB2Field53a(Trade trade) {
		...
		CashFlowSi payCashFlowSi = ((TradeFxForward)trade).getCashFlow(PaymentIndicator.PAY).getCashFlowSi();
	...
	public Field getSequenceB2Field57a(Trade trade) {
		...
		String settlementBankSwiftBic = payCashFlowSi.getSettlementBankSwiftBicCode();
		...
		if (StringUtils.isNotBlank(settlementBankSwiftBic)) {
			...
			String settlementBankNameLine1 = payCashFlowSi.getSettlementBankNameLine1();
			String settlementBankNameLine2 = payCashFlowSi.getSettlementBankNameLine2();
	...
	/**
	 * Format and return Sequence C Field 72 based on the given Forward trade.
	 *
	 * @param trade FX trade that requires generation of MT300
	...
	public Field72 getSequenceCField72(Trade trade) {
		...
		FxSupplementaryInformation fxSupplementaryInfo = ((TradeFxForward) trade).getFxSupplementaryInfo();
	...
	/**
	 * Format and return Sequence E Field 22L based on the given Forward trade.
	 *
	 * @param trade FX trade that requires generation of MT300
	...
	/**
	 * Format and return Sequence E Field 22M based on the given Forward trade.
	 *
	 * @param trade FX trade that requires generation of MT300
	...
	/**
	 * Format and return Sequence E Field 22N based on the given Forward trade.
	 *
	 * @param trade FX trade that requires generation of MT300
	...
	/**
	 * Format and return Sequence E Field 22P based on the given Forward trade.
	 *
	 * @param trade FX trade that requires generation of MT300
	...
	/**
	 * Format and return Sequence E Field 22R based on the given Forward trade.
	 *
	 * @param trade FX trade that requires generation of MT300
